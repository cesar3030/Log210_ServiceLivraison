...
public class UpdateUserAccount extends HttpServlet {
	...
	/**
	 * Cette m������thode va traiter la requete Post qui a ������t������
	 * envoy������ par le formulaire de de modification de compte de la page
	 * ��myAccount.jsp Elle teste la validit������ du format de l'email et du
	 * mot de passe. Si ils ont un bon format, on fait la requete BD. Sinon on
	 * retourne a la page du formulaire et on affiche les erreurs pour que
	 * l'utilisateur se corrige.
	 */
	public void doPost(HttpServletRequest request, HttpServletResponse response)
			throws ServletException, IOException {

		// On cr������������ un UserAccountBean qui va nous servir a stocker le
		// email et le mot de passe saisi dans le formulaire de connexion
		...
		// cette ma va nous servir a socker le r������sultat du test de la
		// validit������ du format de l'email et du mot de passe
		Map<String, String> errors;

		// Je v������rifie le format de l'email et du mot de passe. Si il y a
		// des erreurs, elles seront enregistr������es dans une map
		// qu'on renverra au formulaire et qui affichera le contenu de la map
		errors = user
				.verifyValidityOfDatas(user.getEmail(), user.getPassword());

		// Si les donn������es du formulaires sont au bon format, je fais la
		// requete au serveur
		if (errors.isEmpty()) {
			// Ici on va stocker le retour de l'update(si 0 tout s'est bien
			// pass��, si 1 il y a eu un probl��me)
			int returnedValue = 0;
			...
			if (returnedValue == 1) {
				// on cree une erreur a afficher et on l'ins��re dans la map
				// qu'on utilise pour afficher les erreurs
				errors.put("UpdateFail",
						"Votre compte n'a pas pu etre mis a jours. Veuillez reessayer.");

				// On set la map d'erreurs pour pouvoir les afficher sur le
				// formulaire
				session.setAttribute(ERRORS_FORM, errors);

				// On retourne sur la page de connection pour afficher le
				// formulaire et l'erreur
				this.getServletContext().getRequestDispatcher(MYACCOUNT_PAGE)
						.forward(request, response);
			} else {

				// Si les informations ont ��t�� updat��es dans la BD alors on
				// set le user avec ses valeurs modifi��s
				// a la variable global qui contient le bean de l'utilisateur
				// connect��

				// On r��cup��re l'ancienne version
				UserAccountBean userConnected = (UserAccountBean) session
						.getAttribute("userSession");
				...
				// On set le bean user avec les valeurs modifi��es dan la
				// variable globale
